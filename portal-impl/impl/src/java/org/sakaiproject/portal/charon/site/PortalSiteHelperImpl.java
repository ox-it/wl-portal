...
public class PortalSiteHelperImpl implements PortalSiteHelper
{
			...
			boolean resetTools, boolean includeSummary)
	{
		...
		if (effectiveSiteId.equals("!error")){
			String siteId = null;
			if (req.getParameter("siteId")!=null){
				siteId = req.getParameter("siteId");
			}
			else if (req.getAttribute("siteId")!=null){
				siteId = (String) req.getAttribute("siteId");
			}
			if (siteId!=null){
				feedbackUrl = URLUtils.addParameter(feedbackUrl, "siteId", siteId);
				String pageRefUrl = (String) l.get(0).get("pageRefUrl");
				l.get(0).put("pageRefUrl", URLUtils.addParameter(pageRefUrl, "siteId", siteId));
			}
		}
	...
	private String getFeedbackUrl(String siteId) {
		return ServerConfigurationService.getPortalUrl() + "/site/" + (siteId.isEmpty() ? "" : siteId + "/" ) + "page/" + ServerConfigurationService.getString(SkinnableCharonPortal.CONTACT_US_URL_SUFFIX, SkinnableCharonPortal.CONTACT_US_URL_DEFAULT);
	}
